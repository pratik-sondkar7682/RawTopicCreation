...
service_obj = Service("C:\\Users\\tnluser\\Desktop\\CMS Automation\\chromedriver.exe")
driver = webdriver.Chrome(service=service_obj)
driver.maximize_window()

# Database
df = pd.read_csv("C:\\Users\\tnluser\\Desktop\\CMS Automation\\RT Data.csv")
...
while row < len(df):
    ...
    time.sleep(5)
    print(f'Row No. - {row+2} - {df["MID"][row]} - {df["RT ID"][row]}')
    row = row + 1
    df.to_csv("C:\\Users\\tnluser\\Desktop\\CMS Automation\\output.csv")
    time.sleep(0.5)
